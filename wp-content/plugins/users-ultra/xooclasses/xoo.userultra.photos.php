...
$upload_folder =   $upload_dir['baseurl'].'/'.$xoouserultra->get_option('media_uploading_folder');
	...
	public function reload_galleries ()
	{
		...
		{
			$galleries = $wpdb->get_results( 'SELECT `gallery_id`, `gallery_user_id`, `gallery_name`, `gallery_desc`  FROM ' . $wpdb->prefix . 'usersultra_galleries WHERE `gallery_user_id` = "' . (int)$user_id . '" ORDER BY `gallery_order` ASC' );
	...
	public function reload_videos ()
	{
		...
		if (is_user_logged_in() && isset($user_id)) 
		{
			$videos = $wpdb->get_results( 'SELECT * FROM ' . $wpdb->prefix . 'usersultra_videos WHERE `video_user_id` = "' . (int)$user_id . '" ORDER BY `video_order` ASC' );
	...
	function sort_photo_list() 
	{
		...
		foreach ($order as $item_id) 
		{
			...
			$query = "UPDATE " . $wpdb->prefix ."usersultra_photos SET photo_order = '$counter' WHERE  `photo_id` = '".(int)$item_id."' ";						
		    $wpdb->query( $query );
	...
	function sort_gallery_list() 
	{
		...
		foreach ($order as $item_id) 
		{
			...
			$query = "UPDATE " . $wpdb->prefix ."usersultra_galleries SET gallery_order = '$counter' WHERE  `gallery_id` = '".(int)$item_id."' AND   `gallery_user_id` = '".(int)$user_id."' ";						
		    $wpdb->query( $query );
		...
		if ( empty( $photos ) )
			{
				...
				foreach ( $photos as $photo )
				{
					...
					$thumb = $upload_folder."/".$user_id."/".$photo->photo_thumb;
  ...
  $html .="</select>
	...
	public function edit_photo_confirm ()
	{
		...
		if($photo_id!="")
		{
			//check if photo belons to this user
			
			$query = "UPDATE " . $wpdb->prefix ."usersultra_photos SET `photo_name` = '$photo_name', `photo_desc` = '$photo_desc'  , `photo_tags` = '$photo_tags'  WHERE  `photo_id` = '".(int)$photo_id."' ";
			$wpdb->query( $query );		
			
			//update categories table
			$query = "DELETE FROM " . $wpdb->prefix ."usersultra_photo_cat_rel   WHERE  `photo_rel_photo_id` = '".(int)$photo_id."' ";
			$wpdb->query( $query );
	...
	public function edit_gallery ()
	{
		...
		if($gal_id!="")
		{
			...
			$res = $wpdb->get_results( 'SELECT *  FROM ' . $wpdb->prefix . 'usersultra_galleries WHERE `gallery_id` = ' . (int)$gal_id . ' AND `gallery_user_id` = ' . (int)$user_id . ' ' );
  ...
  <option value='2' ".$friends.">Only Friends</option>
	...
	public function edit_gallery_confirm ()
	{
		...
		if($gal_id!="")
		{
			$query = "UPDATE " . $wpdb->prefix ."usersultra_galleries SET `gallery_name` = '$gal_name', `gallery_desc` = '$gal_desc'  , `gallery_private` = '$gal_visibility'  WHERE  `gallery_id` = '".(int)$gal_id."' AND `gallery_user_id` = '".(int)$user_id."' ";		
	...
	public function edit_video ()
	{
		...
		if($video_id!="")
		{
			...
			$res = $wpdb->get_results( 'SELECT *  FROM ' . $wpdb->prefix . 'usersultra_videos WHERE `video_id` = ' . (int)$video_id . ' AND `video_user_id` = ' . (int)$user_id . ' ' );
  ...
  <option value='vimeo' ".$vimeo.">Vimeo</option>
	...
	public function delete_gallery ()
	{
		...
		if($gal_id!="")
		{
			...
			$photos = $wpdb->get_results( 'SELECT *  FROM ' . $wpdb->prefix . 'usersultra_photos WHERE `photo_gal_id` = ' . (int)$gal_id . ' ' );
			...
			$query = "DELETE FROM " . $wpdb->prefix ."usersultra_galleries WHERE  `gallery_id` = '".(int)$gal_id."' ";						
			$wpdb->query( $query );	
	...
	public function delete_video ()
	{
		...
		if($video_id!="")
		{
			...
			//delete  from db
			$query = "DELETE FROM " . $wpdb->prefix ."usersultra_videos WHERE  `video_id` = '".(int)$video_id."'  AND video_user_id = '".(int)$user_id."' ";						
			$wpdb->query( $query );	
	...
	public function delete_photo ()
	{
		...
		$photos = $wpdb->get_results( 'SELECT *  FROM ' . $wpdb->prefix . 'usersultra_photos WHERE `photo_id` = ' . (int)$photo_id . ' ' );
		...
		$query = "DELETE FROM " . $wpdb->prefix ."usersultra_photos WHERE  `photo_id` = '".(int)$photo_id."' ";						
		$wpdb->query( $query );	
	...
	public function set_as_main_photo ()
	{
		...
		$query = "UPDATE " . $wpdb->prefix ."usersultra_photos SET photo_main = '0' WHERE  `photo_gal_id` = '".(int)$gal_id."' ";						
		$wpdb->query( $query );
		
		//set to main
		$query = "UPDATE " . $wpdb->prefix ."usersultra_photos SET photo_main = '1' WHERE  `photo_id` = '".(int)$photo_id."' AND  `photo_gal_id` = '".(int)$gal_id."' ";						
		$wpdb->query( $query );
		...
		if (isset($gal_id)) 
		{
			...
			if ( empty( $photos ) )
			{
				...
				foreach ( $photos as $photo )
				{
					...
					$thumb = $upload_folder."/".$user_id."/".$photo->photo_thumb;
					$large = $upload_folder."/".$user_id."/".$photo->photo_large;
	...
	public function get_single_photo($photo_id, $user_id, $display_photo_rating, $display_photo_description)
	{
		...
		if (!$error) 
		{
			$thumb = $upload_folder."/".$user_id."/".$current_photo->photo_large;								
	...
	public function reload_photos ()
	{
		...
		if (is_user_logged_in() && isset($user_id)) 
		{
			$photos = $wpdb->get_results( 'SELECT *  FROM ' . $wpdb->prefix . 'usersultra_photos WHERE `photo_gal_id` = "' . (int)$gal_id . '" ORDER BY `photo_order` ASC' );

...
/*Process uploads*/
	function process_cvs($array) 
	{
		...
		if(!is_super_admin( $current_user ))
		{
			$this->messages_process .= '<p> <strong>'.__('--- You cannot import users ---  ', 'xoousers').'</strong></p>';
			return;
			...
			foreach ($_FILES as $key => $array) {
				...
				if ($name) {
					...
					if ( !in_array($ext, $this->allowed_extensions) )
					{
						...
						$upload_dir = wp_upload_dir(); 
						$path_f =   $upload_dir['basedir'].'/'.$xoouserultra->get_option('media_uploading_folder');
	...
	public function create_user_import ($user, $headers, $send_welcome_email, $account_status, $count)
	{
		...
		{			
			...
			if ( ! $user_id ) 
			{
				...
				update_user_meta ($user_id, 'first_name', sanitize_text_field($f_name));
				update_user_meta ($user_id, 'last_name', sanitize_text_field($l_name));								
				...
				if($columns > 5)
				{
					...
					for($i=5; $i<$columns; $i++):
									if(in_array($headers[$i], $this->wp_users_fields))
										...
										update_user_meta($user_id, $headers[$i], sanitize_text_field($user[$i]));
					endfor;

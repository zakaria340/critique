...
class XooUserAdmin extends XooUserUltraCommon 
{
	...
	function userultra_set_default_options()
	{
		...
		$this->userultra_default_options = array(
						...
						'media_uploading_folder' => 'usersultramedia',
	...
	function create_uploader_folder ()
	{
		...
		$wp_content_path  = $upload_dir['basedir']."/";
	...
	function checkUploadFolder()
	{
		...
		if(!is_dir($path_pics))
		{
			...
			$html .= ' <div ><div class="user-ultra-warning">'.__("Please create '".$media_folder."' folder with 0755 attribute in your uploads folder. You can create it automatically by <a href='#' id='uultradmin-create-upload-folder'>clicking here</a>", 'xoousers').'</div></div>';
			
			$html .= ' <div ><div class="user-ultra-warning">'.__("IMPORTANT: If you've already created the '".$media_folder."' folder, please move it within your uploads folder. This is required on version 1.5.63(12-06-2015). This should affect only already users. Also, make sure that the upload folder on UU Settigns is '".$media_folder."' instead 'wp-content/".$media_folder."'", 'xoousers').'</div></div>';

	...
	var colpick = function () {
		var
			...
			downHue = function (ev) {
				...
				jQuery(document).on('mouseup touchend',current,upHue);
				jQuery(document).on('mousemove touchmove',current,moveHue);
			...
			upHue = function (ev) {
				...
				jQuery(document).off('mouseup touchend',upHue);
				jQuery(document).off('mousemove touchmove',moveHue);
				return false;
			},
			...
			downSelector = function (ev) {
				...
				jQuery(document).on('mouseup touchend',current,upSelector);
				jQuery(document).on('mousemove touchmove',current,moveSelector);
			...
			upSelector = function (ev) {
				fillRGBFields(ev.data.cal.data('colpick').color, ev.data.cal.get(0));
				fillHexFields(ev.data.cal.data('colpick').color, ev.data.cal.get(0));
				jQuery(document).off('mouseup touchend',upSelector);
				jQuery(document).off('mousemove touchmove',moveSelector);
				return false;
			},
			//Submit button

    ...
    if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'postratings') {
        ...
        if ($rate > 0 && $post_id > 0 && check_allowtorate()) {
            ...
            if ($rated) {
                ...
                $post_ratings_score = ($post_ratings_score - intval($get_rated->rating_rating));
...
function mine_expand_ratings_template($template, $post_data, $post_ratings_data = null, $max_post_title_chars = 0, $is_main_loop = true) {
    global $post;
    ...
    if (strpos($template, '%RATINGS_IMAGES%') !== false) {
        $post_ratings_images = '<tag>'.mine_get_ratings_images_vote($ratings_custom, $ratings_max, $post_ratings, $ratings_image, $post_ratings_alt_text, $insert_half).'</tag>';

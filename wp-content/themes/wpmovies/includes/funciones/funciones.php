...
<?php
$meta_datas = get_post_meta( get_the_ID() );
$season_rows = array();
if(!empty($meta_datas))
{
	foreach($meta_datas as $key => $meta_data){
		preg_match('/^seasons_\d+_episode_\d+_url_tvshows/is',$key,$se);
		if($se){
			if(!empty($meta_data)){
				$season_no = preg_replace('/_episode(.*)/','',$key);
				$season_no = preg_replace('/(.*)seasons_/','',$season_no);
				$episode_no = preg_replace('/(.*)episode_/','',$key);
				$episode_no = preg_replace('/_url_tvshows(.*)/','',$episode_no);
				$season_rows[$season_no][$episode_no] = $meta_data;
			}			
		}
...
<div id='cssmenu'>
<ul>
    <?php   foreach($season_rows as $season => $season_row){   ?>
	<li class='has-sub'><a href='#'><span><b class="icon-bars"></b> <?php if($tex = get_option('text-41')) { echo $tex; } else { _e('Season','mundothemes'); } ?> <?php echo $season; ?></span></a>
	     <ul>
		 <?php if(!empty($season_row)){ ?>
		 <?php $numerado2 = 1;  foreach($season_row as $episode => $episode_url){  ?> 
		 <li>
		 <?php if($episode_url) { ?>
		 <a href="<?php bloginfo('url'); ?>/watch-episodios/<?php echo $episode_url[0]; ?>/" target="_blank">
		 <?php } else { ?>
		 <a>
		 <?php } ?>       
		 <span class="datex"><?php echo $season; ?> - <?php echo $episode; ?></span>
		 <span class="datix"><b class="icon-chevron-right"></b> 
		 <?php if($dato = get_sub_field('title_tvshows')) { ?>
		 <?php echo $dato; ?>
		 <?php } else { ?> 
		 <?php if($tex = get_option('text-42')) { echo $tex; } else { _e('Episode','mundothemes'); } ?> <?php echo $episode; ?>
		 <?php } ?>
		 </span>

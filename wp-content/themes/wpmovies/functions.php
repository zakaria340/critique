...
function edd_sample_theme_license_page() {
	update_option('edd_sample_theme_license_key','Licensed');
...
<h2><?php _e('Mundothemes License','mundothemes'); ?></h2>
<a href="#" target="_blank">
<img class="mundothemes" src="<?php echo get_stylesheet_directory_uri()."/includes/framework/"; ?>images/logo.png">
</a>
</div>
...
if( isset( $_POST['edd_theme_license_activate'] ) ) {
		...
		update_option('edd_sample_theme_license_key','Licensed');
	    update_option('edd_sample_theme_license_key_status','valid');
		//update_option( 'edd_sample_theme_license_key_status', 'valid');//$license_data->license );
...
register_taxonomy('tvshows_categories', array('tvshows',),
array('show_admin_column' => true, 'hierarchical' => true, 'rewrite' => array('slug' => get_option('tvshows-category')),)
);
}
...
function tvshows() {
	...
	$args = array(
		'label'               => __( 'tvshows', 'mundothemes' ),
		'description'         => __( 'Post Type Description', 'mundothemes' ),
		'labels'              => $labels,
		'supports'            => array( 'title', 'editor', 'excerpt', 'author', 'thumbnail', 'comments', 'trackbacks', 'revisions', 'custom-fields', 'page-attributes', 'post-formats' ),
		'taxonomies'          => array( 'tvshows_categories', get_option('year'), get_option('actor'), get_option('director'),  ),
		'hierarchical'        => false,
		'public'              => true,
		'show_ui'             => true,
		'show_in_menu'        => true,
...
register_taxonomy('news_categories', array('news',),
array('show_admin_column' => true, 'hierarchical' => true, 'rewrite' => array('slug' => get_option('news-category')),)
);
}
...
function noticias() {
	$labels = array(
		...
		'singular_name'       => _x( 'News', 'Post Type Singular Name', 'mundothemes' ),
...
function wpshed_meta_box_save( $post_id ) {
	...
	if( isset( $_POST['titulo_serie'] ) )
		update_post_meta( $post_id, 'titulo_serie', esc_attr( $_POST['titulo_serie'] ) );
	if( isset( $_POST['url_serie'] ) )
		update_post_meta( $post_id, 'url_serie', esc_attr( $_POST['url_serie'] ) );
	if( isset( $_POST['fecha_serie'] ) )
		update_post_meta( $post_id, 'fecha_serie', esc_attr( $_POST['fecha_serie'] ) );
	if( isset( $_POST['temporada_serie'] ) )
		update_post_meta( $post_id, 'temporada_serie', esc_attr( $_POST['temporada_serie'] ) );
	if( isset( $_POST['episodio_serie'] ) )
		update_post_meta( $post_id, 'episodio_serie', esc_attr( $_POST['episodio_serie'] ) );
}
